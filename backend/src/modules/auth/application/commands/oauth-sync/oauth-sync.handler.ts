...
export class OAuthSyncHandler implements ICommandHandler<OAuthSyncCommand, OAuthSyncResult> {
  constructor(
  ...
  async execute(command: OAuthSyncCommand): Promise<OAuthSyncResult> {
    ...
    const claims = await this.authProvider.verifyAccessToken(command.accessToken);
