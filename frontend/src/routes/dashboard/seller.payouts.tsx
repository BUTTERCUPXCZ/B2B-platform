...
import { sellerSalesPeriods } from "@/components/dashboard/dashboard-data"
import { cn } from "@/lib/utils"

export const Route = createFileRoute("/dashboard/seller/payouts")({
  component: SellerSalesRoute,
})
...
function SellerSalesRoute() {
  const [open, setOpen] = useState<string | null>(sellerSalesPeriods[0]?.id ?? null)

  const sales30d = sellerSalesPeriods
    .slice(0, 2)
    .reduce((s, p) => s + p.netToBank, 0)
  const lifetimeGross = sellerSalesPeriods.reduce((s, p) => s + p.gross, 0)
  const lifetimeCommission = sellerSalesPeriods.reduce((s, p) => s + p.commission, 0)
  const bankMasked = sellerSalesPeriods[0]?.bankMasked ?? "—"

  return (
    <ShadcnDashboardShell role="seller" title="Sales">
      <div className="grid grid-cols-2 gap-4 px-4 lg:grid-cols-4 lg:px-6">
        ...
        <Card>
          <CardHeader>
            ...
            <p className="text-xs text-muted-foreground">All buyer payments deposit here</p>
          </CardHeader>
        </Card>
      </div>
        ...
        <div className="flex items-start gap-3 rounded-xl border border-border bg-amber-50/40 p-4">
          ...
          <div className="text-sm text-brand-black/80">
            <p className="font-semibold text-brand-black">
              Buyer payments deposit directly to your bank.
            </p>
      ...
      <SubPage
        title="Sales history"
        description="Each row is a fortnightly settlement summary. Expand to see per-order detail."
      >
        <Table>
          <TableHeader>
            <TableRow>
              ...
              return (
                <Fragment key={period.id}>
                  ...
                  >
                    ...
                    <TableCell className="text-right">{period.orders}</TableCell>
