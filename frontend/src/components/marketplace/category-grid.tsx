...
const categories: Category[] = [
  { icon: BrickWallIcon, name: "Cement & Concrete", count: "2,400+ items" },
  ...
  { icon: Tap01Icon, name: "Plumbing", count: "1,940+ items" },
  ...
  { icon: Plant02Icon, name: "Landscaping", count: "1,070+ items" },
]

export function CategoryGrid() {
  return (
    <section id="categories" className="bg-white py-14 sm:py-16 lg:py-20" aria-labelledby="categories-heading">
      <div className="mx-auto max-w-[1440px] px-6">
        <Reveal className="mb-10 flex flex-col items-start justify-between gap-3 sm:flex-row sm:items-end">
          <div>
            ...
            <h2 id="categories-heading" className="mt-2 text-3xl font-extrabold tracking-tight text-brand-black sm:text-4xl">
        ...
        <StaggerGroup className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-6">
          {categories.map((c) => (
            ...
            >
              <a href={`/shop?category=${encodeURIComponent(c.name)}`} aria-label={`Browse ${c.name}`} className="absolute inset-0 rounded-lg transition-shadow hover:shadow-lg focus-visible:ring-2 focus-visible:ring-brand-orange focus-visible:ring-offset-2" />
